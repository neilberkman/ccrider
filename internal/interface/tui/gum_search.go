...
func (s *pagerSearch) Execute(m *pagerModel) {
	defer s.Done()
	if s.input.Value() == "" {
		s.query = nil
		return
...
func (s *pagerSearch) NextMatch(m *pagerModel) {
...
func (s *pagerSearch) PrevMatch(m *pagerModel) {
	// Check that we are within bounds.
	if s.query == nil {
		return
	}
