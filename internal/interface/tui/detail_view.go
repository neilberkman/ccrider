...
func openInNewTerminal(sessionID, projectPath, lastCwd, updatedAt, summary string) tea.Cmd {
	return func() tea.Msg {
		// Load config to get terminal command
		cfg, err := config.Load()
		if err != nil {
		...
		shellCmd, err := session.BuildResumeCommand(sessionID, projectPath, lastCwd, updatedAt, false)
		if err != nil {
			return terminalSpawnedMsg{
				success: false,
				message: "Failed to build command",
				err:     fmt.Errorf("build command: %w", err),
			}
		}
